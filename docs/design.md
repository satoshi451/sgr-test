...
## Текущий статус
- Создан каркас uv-проекта с FastAPI и заготовкой сервисов (`metadata`, `categories`, `reasoning`).
- Настроена конфигурация через `pydantic` с параметрами Trino, whitelist и приватного LLM.
- API-слой предоставляет черновые эндпоинты `/api/schema`, `/api/categories`, `/api/query`, `/api/execute`.
- Интеграция с `sgr-agent-core` предусмотрена через опциональный клиент; при отсутствии зависимостей эндпоинт возвращает подсказку о необходимости настроить LLM.
