...
## API (черновик)
- `GET /api/schema` — получить разрешённые схемы/таблицы/поля (использует файловый кеш, можно `?refresh=true`).
- `GET /api/categories` — список уникальных значений для столбца.
- `POST /api/query` — сгенерировать SQL из пользовательского запроса (SGR), поле `refresh_schema` принудительно обновляет кеш схемы.
- `POST /api/execute` — выполнить только SELECT-запрос (с автоматическим LIMIT, запрет DDL/DML).
